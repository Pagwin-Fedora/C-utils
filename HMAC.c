...
void (*intern_free)(void*) = free;
void setAlloc(void* (*alloc)(size_t)){
    allocator = alloc;
}
void setDealloc(void (*intern_free)(void*) dealloc){
    intern_free = dealloc;
}
char* HMAC(void* (*hash_func)(void*,size_t),size_t hash_func_block_size,size_t hash_func_digest_size,char* password, size_t password_len, char* data, size_t data_len){
    //Function for getting the HMAC Block(inside the HMAC function to prevent user usage)
    void* HMAC_BLOCK(void* input,size_t input_len, size_t block_size, size_t digest_size,void* (*hasher)(void*,size_t)){
	char* output = (*allocator)(block_size);
    ...
    char* opad_block = (*allocator)(hash_func_block_size);
    for(size_t i = 0;i<hash_func_block_size;i++){
	opad_block[i] = 0x5c^block[i];
    }
    //generating the ipadding
    char* ipad_block = (*allocator)(hash_func_block_size+data_len);
    for(size_t i = 0;i<hash_func_block_size;i++){
    ...
    (*intern_free)(ipad_block);
    
    //gathering the hash input all toegether
    void* hash_input = (*allocator)(hash_func_block_size+hash_func_digest_size);
    ...
    //taking the final hash for our output
    ...
    (*intern_free)(hash_input);
    (*intern_free)(opad_block);
    (*intern_free)(ipad_output);
...
char* hexify(unsigned char* bin_data,size_t data_len){
	...
	char* output = (*allocator)(data_len*2);
