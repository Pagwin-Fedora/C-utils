...
//Function for getting the HMAC Block(was inside the HMAC function to prevent user usage but that seems to that seems to not be fully portable so I moved it out) Edit: past me doesn't know what header files are for, the reason it's inside this function is to make it clear it's only used here
inline void* HMAC_BLOCK(void* input,size_t input_len, size_t block_size, size_t digest_size,void* (*hasher)(void*,size_t)){
